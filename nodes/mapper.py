...
class MapperNode(Node):
    def __init__(self):
        ...
        self.pool_size_x = None
        self.pool_size_y = None
        self.cell_resolution = None
        self.init_params()

        self.num_cells_x = int(
            np.round(self.pool_size_x / self.cell_resolution)
        )
        self.num_cells_y = int(
            np.round(self.pool_size_y / self.cell_resolution)
        )
        ...
        self.map = self.get_pool_walls(empty_map)
        ...
        self.map_pub = self.create_publisher(
            msg_type=OccupancyGrid, topic='occupancy_grid', qos_profile=1
        )
    ...
    def init_params(self):
        self.declare_parameters(
            namespace='',
            parameters=[
                ('pool_size_x', rclpy.Parameter.Type.DOUBLE),
                ('pool_size_y', rclpy.Parameter.Type.DOUBLE),
                ('cell_resolution', rclpy.Parameter.Type.DOUBLE),
            ],
        )
        param = self.get_parameter('pool_size_x')
        self.get_logger().info(f'{param.name}={param.value}')
        self.pool_size_x = param.value
        param = self.get_parameter('pool_size_y')
        self.get_logger().info(f'{param.name}={param.value}')
        self.pool_size_y = param.value
    ...
    def include_obstacle_in_map(
        self, obstacle: Polygon, cells: np.ndarray
    ) -> np.ndarray:
        ...
        for index, point in enumerate(obstacle.points):
            obstacle_cells[index, 0] = int(
                round(point.x / self.cell_resolution)
            )
            obstacle_cells[index, 1] = int(
                round(point.y / self.cell_resolution)
            )

        img = cells.astype(dtype='uint8')
        img_obstacles = cv2.fillPoly(
            img, pts=np.int32([obstacle_cells]), color=(OCCUPIED)
        )
    ...
    def inflate_obstacles(self, cells: np.ndarray) -> np.ndarray:
        ...
        kernel = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (dilatation_size, dilatation_size)
        )
    ...
    def get_pool_walls(self, cells: np.ndarray) -> np.ndarray:
