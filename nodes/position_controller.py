...
class PositionController(Node):
    def __init__(self):
        ...
        self.p_component_pub = self.create_publisher(
            msg_type=Vector3Stamped, topic='~/p_component', qos_profile=1
        )

        self.i_component_pub = self.create_publisher(
            msg_type=Vector3Stamped, topic='~/i_component', qos_profile=1
        )

        self.d_component_pub = self.create_publisher(
            msg_type=Vector3Stamped, topic='~/d_component', qos_profile=1
        )

        self.d_component_unfiltered_pub = self.create_publisher(
    ...
    def publish_thrust(
        self, thrust: np.ndarray, timestamp: rclpy.time.Time
    ) -> None:
    ...
    def apply_control(
        ...
        d_component_unfiltered = (
            R.transpose() @ self.K_d @ (self.dsetpoint - dposition)
        )
