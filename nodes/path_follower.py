...
class Trajectory:
    def __init__(
        ...
        # for the positions, assign time stamps to the positions such that a
        # "smooth" trajectory from rest to rest is generated
        self.positions = [
            np.array(
                [
        ...
        # for yaw, only take first and last orientation, as intermediate frames
        # don't matter (not important for obstacles)
        # a yaw-trajectory is then generated, considering maximum allowed
        # angular velocity / angular acceleration such that path
        ...
        mutual_distances = [
            distance(path[i].pose, path[i + 1].pose)
            for i in range(len(path) - 1)
        ]
        self.cumulative_distances = [0.0] + [
            sum(mutual_distances[0 : i + 1])
            for i in range(len(mutual_distances))
        ]
        # calculation of path segments as in
        # https://www.osrobotics.org/osr/planning/time_parameterization.html,
        # a setpoint on the trajectory is then calculated from the progress of
        # distance along the scalar path trajectory
        ...
        # check if resulting yaw trajectory is within limits and scale path
        # trajectory accordingly,
        # depending on bounds on velocity and acceleration determine the
        # number of path segments
        time_scaling = 1.0
        path_keypoints = [
            self.path_trajectory.path_variable(t)
            for t in self.path_trajectory.key_times
        ]
        yaw_keypoints = [
            keypoint * (self.yawbounds[1] - self.yawbounds[0])
            + self.yawbounds[0]
            for keypoint in path_keypoints
        ]
        if len(path_keypoints) == 3:
            acceleration = (
                2
                * (yaw_keypoints[0] - self.yawbounds[0])
                / np.power(self.path_trajectory.key_times[0], 2)
            )
            time_scaling = max(
                time_scaling, np.sqrt(abs(acceleration / max_yaw_acceleration))
            )
            velocity = (yaw_keypoints[1] - yaw_keypoints[0]) / (
                self.path_trajectory.key_times[1]
                - self.path_trajectory.key_times[0]
            )
            time_scaling = max(time_scaling, abs(velocity / max_yaw_velocity))
            deceleration = abs(
                2
                * (yaw_keypoints[2] - yaw_keypoints[1])
                / np.power(
                    self.path_trajectory.key_times[2]
                    - self.path_trajectory.key_times[1],
                    2,
                )
            )
            time_scaling = max(
                time_scaling, np.sqrt(abs(deceleration / max_yaw_acceleration))
            )
    ...
    def get_pose(self, t):
        ...
        while True:
            if (
                path_variable * self.cumulative_distances[-1]
                <= self.cumulative_distances[idx]
            ):
                break
        ...
        position = self.positions[idx - 1] + (
            ...
            path_variable * self.cumulative_distances[-1]
            - self.cumulative_distances[idx - 1]
        ) / (
            self.cumulative_distances[idx] - self.cumulative_distances[idx - 1]
        )
        yaw = self.yawbounds[0] + path_variable * (
            self.yawbounds[1] - self.yawbounds[0]
        )
        return position, yaw


class PathTrajectory:
    # scalar rest-to-rest-trajectory consisting of a constant acceleration, a
    # constant velocity and a constant deceleration segment
    # https://www.osrobotics.org/osr/planning/time_parameterization.html
    def __init__(self, ds_max, dds_max):
        ...
        if ds_max >= np.sqrt(
            dds_max
        ):  # acceleration up to time stamp ts, deceleration from ts to 2*ts
    ...
    def path_variable(self, t):
        ...
        if len(self.key_times) == 2:
            return (
                self.dds_max * self.key_times[0] ** 2
                - 1 / 2.0 * self.dds_max * (self.key_times[1] - t) ** 2
            )
        # acceleration up to time stamp t0, constant velocity between
        # t0 and t0 + t1, deceleration up to 2 * t0 + t1
        else:
            if t < self.key_times[1]:
                return 1 / 2.0 * self.dds_max * self.key_times[
                    0
                ] ** 2 + self.ds_max * (t - self.key_times[0])
            else:
                return (
                    self.dds_max * self.key_times[0] ** 2
                    - 1 / 2.0 * self.dds_max * (self.key_times[2] - t) ** 2
                    + self.ds_max * (self.key_times[1] - self.key_times[0])
                )
...
class PathFollower(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            namespace='',
            parameters=[
                ('max_velocity', rclpy.Parameter.Type.DOUBLE),
        ...
        self.yaw_pub = self.create_publisher(
            YawTarget, 'yaw_controller/setpoint', 1
        )
        ...
        self.trajectory_timer = self.create_timer(
            1 / frequency, self.send_setpoint
        )

    def init_services(self):
        self.set_path_service = self.create_service(
            SetPath, '~/set_path', self.serve_set_path
        )
    ...
    def send_setpoint(self):
        ...
        msg = YawTarget()
        msg.yaw_target = self.target_yaw
        msg.header.stamp = stamp
        msg.header.frame_id = 'map'
        self.yaw_pub.publish(msg)

        position = geometry_msgs.msg.Point(
            x=self.target_position[0, 0],
            y=self.target_position[1, 0],
